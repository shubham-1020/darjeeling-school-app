...
// Log environment variables BEFORE creating the ImageKit instance
console.log("ImageKit Public Key:", import.meta.env.VITE_IMAGEKIT_PUBLIC_KEY);
console.log("ImageKit URL Endpoint:", import.meta.env.VITE_IMAGEKIT_URL_ENDPOINT);
console.log("ImageKit Auth Endpoint (original):", "http://localhost:5001/auth"); // Keep this for reference


// Initialize ImageKit WITHOUT the authenticationEndpoint for now
// We will manually fetch the tokens and pass them directly.
...
export default function NoticeImageUpload({ onImageUploadSuccess, onImageUploadError }) {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      // STEP 1: Manually fetch authentication parameters from your backend
      console.log("Attempting to FETCH authentication parameters from backend...");
      const authResponse = await axios.get(`${API_URL}/auth`);
      ...
      console.log("Authentication parameters FETCHED successfully:", { token, signature, expire });

      // STEP 2: Use the fetched parameters directly in imagekit.upload()
      console.log("Attempting ImageKit upload with EXPLICIT authentication parameters...");
      const res = await imagekit.upload({
        file,
        fileName: file.name,
        tags: ["notice-image"],
        // Pass the fetched authentication parameters directly here:
        token: token,
        signature: signature,
        expire: expire,
      });
      ...
      onImageUploadSuccess?.(res.url || res.filePath || res);
      setSelectedFileName(""); // Clear file name on successful upload
    } catch (err) {
      console.error("Upload error caught in NoticeImageUpload (during fetch or upload):", err);
      // More detailed error message for the user
      if (err.response) {
        setError(`Failed to fetch auth from backend: ${err.response.status} ${err.response.statusText}`);
      } else if (err.request) {
        setError("Network Error: Could not reach backend authentication server. Check if backend is running and CORS.");
      } else {
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragOver && !uploading ? 'border-blue-500 bg-blue-50' : ''}    // Drag-over specific styles
        ${!isDragOver && !error ? 'hover:border-blue-400 hover:bg-blue-50' : ''} // Hover styles
    ...
    >
      ...
      {uploading ? (
        ...
        <div className="flex flex-col items-center text-blue-600">
          {/* SVG Spinner Icon (Tailwind CSS-compatible) */}
          <svg className="animate-spin h-8 w-8 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <p className="mt-3 text-lg font-medium">Uploading...</p>
          <p className="text-sm text-gray-500">Please wait while your image is being uploaded.</p>
        </div>
      ) : (
        // Default State: Shows upload icon, drag/drop text, and a "Select File" button
        <>
          {/* SVG Upload Icon (Tailwind CSS-compatible) */}
      ...
      {error && (
        ...
        <p className="mt-4 text-sm font-medium text-red-600 flex items-center">
          {/* Error Icon (Tailwind CSS-compatible) */}
